import { State, Store as Store$1, StoreState } from "@ariakit/store";
import { PickByValue, SetState } from "@ariakit/utils";
...
type StateStore<T = Store$1> = T | null | undefined;
type StateKey<T = Store$1> = keyof StoreState<T>;
/**
 ...
 * ```
 */
declare function useStoreState<T extends Store$1>(store: T): StoreState<T>;
declare function useStoreState<T extends Store$1>(store: StateStore<T>): StoreState<T> | undefined;
declare function useStoreState<T extends Store$1, K extends StateKey<T>>(store: T, key: K): StoreState<T>[K];
declare function useStoreState<T extends Store$1, K extends StateKey<T>>(store: StateStore<T>, key: K): StoreState<T>[K] | undefined;
declare function useStoreState<T extends Store$1, V>(store: T, selector: (state: StoreState<T>) => V): V;
declare function useStoreState<T extends Store$1, V>(store: StateStore<T>, selector: (state?: StoreState<T>) => V): V;
type StoreStateObject<T extends StateStore, S extends StoreState<T> | undefined> = Record<string, StateKey<T> | ((state: S) => any)>;
type StoreStateObjectResult<T extends StateStore, S extends StoreState<T> | undefined, O extends StoreStateObject<T, S>> = { [K in keyof O]: O[K] extends keyof StoreState<T> ? O[K] extends keyof S ? S[O[K]] : StoreState<T>[O[K]] | undefined : O[K] extends ((state: S) => infer R) ? R : never };
/**
 * Receives an Ariakit store object (which can be `null` or `undefined`) and
 ...
 */
declare function useStoreStateObject<T extends Store$1, O extends StoreStateObject<T, StoreState<T>>>(store: T, object: O): StoreStateObjectResult<T, StoreState<T>, O>;
declare function useStoreStateObject<T extends StateStore, O extends StoreStateObject<T, StoreState<T> | undefined>>(store: T, object: O): StoreStateObjectResult<T, StoreState<T> | undefined, O>;
/**
 ...
 */
declare function useStoreProps<S extends State, P extends Partial<S>, K extends keyof S, SK extends keyof PickByValue<P, SetState<P[K]>>>(store: Store$1<S>, props: P, key: K, setKey?: SK): void;
/**
 ...
 */
declare function useStore<T extends Store$1, P>(createStore: (props: P) => T, props: P): readonly [T & {
  useState: UseState<StoreState<T>>;
}, () => void];
type Store<T extends Store$1 = Store$1> = T & {
  /**
   * Re-renders the component when the state changes and returns the current
   * state.
   * @deprecated Use
...
//#endregion
export { Store, UseState, useStore, useStoreProps, useStoreState, useStoreStateObject };
